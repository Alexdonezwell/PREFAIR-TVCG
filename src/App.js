...
class App extends React.Component {
      ...
      eachRules: [
        {
          ...
          matchedIds: [
            ...
            22, 23, 24, 25, 26, 27, 28, 29, 30, 33, 35, 37, 38, 39, 40, 41, 42,
            45, 46, 47, 48, 49, 50, 51, 53, 54, 55, 56, 57, 58, 59, 60, 62, 63,
            64, 65, 66, 67, 68, 69, 71, 72, 73, 74, 75, 76, 83, 85, 88, 89, 90,
            91, 92, 94, 97, 98, 99, 101, 103, 104, 107, 108, 109, 111, 112, 113,
            115, 117, 118, 119, 121, 122, 123, 124, 126, 130, 131, 132, 133,
            136, 137, 140, 141, 142,
          ],
          matchedIdsCount: 103,
          pred: null,
  ...
  updateWindowDimensions() {
    ...
    self.setState({
      contentRightHeight1: step1height + step2height + 101.5,
      contentRightHeight2: height2,
  ...
  hightlightTreeData = () => {
    ...
    axios
      .post(apiDomain + "passInteractions", parameters)
      .then(function (resp) {
        if (resp.status == 200) {
          const centers = resp.data.centers;
          let radarImgData = [];
  ...
  getStep3Option() {
    ...
    for (let index in step1data) {
      ...
      if (this.checkTargetInArr(newarr, d["row"])) {
        ...
        if (index == this.state.hoveredStep1Line) {
          color = "black";
          width = 3;
        }
    ...
    var schema = [
      ...
      { name: "statistical_parity_difference", index: 3, text: "SPD" },
      { name: "equal_opportunity_difference", index: 4, text: "EOD" },
      { name: "disparate impact", index: 5, text: "DI" },
      { name: "error_rate_ratio", index: 6, text: "ERR" },
    ];
    ...
    return {
      parallelAxis: [
        ...
        { dim: 1, name: schema[1].text, scale: true, min: 0.8, max: 0.9 },
  ...
  getNamesNode = (name, node) => {
    if (
      node.data.name.indexOf("Clusters") > -1 ||
      node.data.name.indexOf("Raw Data") > -1
    ) {
      return name;
    } else {
  ...
  hoverNode = (currentNode) => {
    ...
    axios.post(url, p).then(function (resp) {
      ...
      dd.push(obj);
      // yusj, match this.state.step1Data to get the index of the selected item, so that we can highlight the line in parallel plot
      let step1Data = that.state.step1Data;
      ...
      for (let i = 0; i < step1Data.length; i++) {
        if (
          ...
          console.log("found");
          self.setState({
            hoveredStep1Line: selectedStep1Line,
          });
          break;
        }
      }
  ...
  onRadarEventsClick(param, self) {
    ...
    this.state.radarColorList[selectedIndex] =
      !this.state.radarColorList[selectedIndex];
    self.clear();
    const radarImgData = this.state.radarImgData;
    const radarRuleData = this.state.radarRuleData;
    ...
    if (scatterSelectedItemData != null) {
      ...
      for (let i = 0; i < radarList.length; i++) {
        ...
        const maxVal = Math.max(...colData);
        normalData.push(
          (scatterSelectedItemData[key] - minVal) / (maxVal - minVal)
        );
      }
  ...
  onSelectedRuleListItems(ids, groupId) {
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        ml_result.eachRules = [
          ml_result.eachRules.find((x) => x.pred_label == 1),
        ];
        self.setState({
  ...
  onUpdatedRuleValue(self, rule, ruleItemsData) {
    ...
    for (let p = 0; p < radarList.length; p++) {
      ...
      for (let c = 0; c < numberOfLines; c++) {
        const poi =
          ((maxFeaturePoint - minFeaturePoint) / numberOfLines) * c +
          minFeaturePoint;
        let row = [];
        for (let i = 0; i < radarList.length; i++) {
    ...
    for (let i = 0; i < groups.length; i++) {
      ...
      const matchedGroupItems = matchedData.filter((item) =>
        groups[i].some((gid) => item["id"] == gid)
      );
      ...
      for (let j = 0; j < matchedGroupItems.length; j++) {
        ...
        for (let k = 0; k < tags.length; k++) {
          const matches = currentTreeData.children.filter(
            (item) => item.name == tags[k]
          );
          if (matches.length <= 0) {
            break;
          }
              ...
              if (i == index) {
                ...
                for (let k = 0; k < rst.length; k++) {
                  ...
                  if (k == 1) {
                    ...
                    for (let item in children) {
                      if (children[item]["name"].trim() == da) {
                        newarr["children"][level1]["children"][item][
                          "highLight"
                        ] = 1;
                        level2 = item;
                      }
  ...
  render() {
    ...
    function scatterOnClick(param) {
      // yusj scatter point click
      console.log(self.state.step1Data);
      console.log(param);
    ...
    return (
      <div id="main">
        <ReactTooltip id="first_tip" place="bottom" type="dark" effect="solid">
          <span>
            {" "}
            1. Select the operations you are interested in applying on the
            dataset
            <br />
            2. Customize the filters on each axis to narrow down the option
            space
            <br />
            3. Click on "Run Clustering" button to group qualified stratrgies
        ...
        <ReactTooltip id="third_tip" place="bottom" type="dark" effect="solid">
          <span>
            {" "}
            By clicking on each cluster tab, a rule list will be generated to
            show how the cluster you select is different from others.{" "}
          </span>
        </ReactTooltip>
        <ReactTooltip id="six_tip" place="bottom" type="dark" effect="solid">
          <span>
            1. You will see strategies in each cluster presented in the form of
            a tree view. <br />
            2. Each branch of the tree view indicates a series of pre-processing
            operations applied on the datasets. <br />
            3. By hovering on each node, on the radar plot you will see the
            performance of applying this strategy on the dataset using the
            provided model.
          </span>
        </ReactTooltip>
        <ReactTooltip
        ...
        >
          <span>
            1. Click on the root node to create new empty child nodes. <br />
            2. Drag the operations from above into the empty slot, you can build
            your customized pre-processing strategy. <br />
            3. Click on the leaf node of your strategy and click the "View
            Result" button. you can see the performance of the current strategy
            in the radar plot.
          </span>
        </ReactTooltip>
          ...
          <div className="other">
            <div className="other-left">
              <div className="step1">
                <div className="title">
                  <div className="text">
                    1.Pre-processing Operation Filter Panel{" "}
                    <a data-tip data-for="first_tip" style={{ color: "black" }}>
                      {" "}
                      <QuestionCircleOutlined />{" "}
                    </a>{" "}
                  </div>
              ...
              <div className="step2">
                <div className="title">
                  <div className="text">
                    2.Clusters and Radar Plot View{" "}
                    <a data-tip data-for="sec_tip" style={{ color: "black" }}>
                      {" "}
                      <QuestionCircleOutlined />{" "}
                    </a>
                  </div>
                ...
                <div
                  ...
                  style={{ height: this.state.step2Height }}
                >
              ...
              <div className="step4">
                <div className="title">
                  <div className="text">
                    3. Descriptive Rule view{" "}
                    <a data-tip data-for="third_tip" style={{ color: "black" }}>
                      {" "}
                      <QuestionCircleOutlined />{" "}
                    </a>
                    <div id="scatterSelectButtonArea">
                      {" "}
                      {this.state.scatterSelectButton}{" "}
                    </div>
                  </div>
                ...
                <div className="content">
                  ...
                  <div
                    className="content scroll scrollx"
                    style={{ height: this.state.contentRightHeight2 + 36 }}
                  >
                    {/* <div className='content-left' style={{ width: '40%', marginRight: '40px', maxWidth: '280px' }}> */}
                    {/* <div className='content'> */}
                    <RuleListComponment
                      onUpdatedRuleValue={this.onUpdatedRuleValue}
                      {...{
                        ruleItemsData: ruleItemsData2,
                        mlStdResult: this.state.ruleStdResultData,
            ...
            <div className="other-right">
              <div className="step6">
                <div className="title">
                  <div className="text" style={{ width: "100%" }}>
                    4.Tree View of Strategy Clusters{" "}
                    <a data-tip data-for="six_tip" style={{ color: "black" }}>
                      {" "}
                      <QuestionCircleOutlined />{" "}
                    </a>
                  </div>
                ...
                <div className="content">
                  <div
                    className="scroll"
                    style={{ height: this.state.contentRightHeight1 }}
                  >
                    {treeOptions}
                  </div>
                </div>
              ...
              <div className="step7">
                <div className="title">
                  <div className="text" style={{ width: "100%" }}>
                    5.Customization View
                    <a
                      data-tip
                      data-for="seventh_tip"
                      style={{ color: "black" }}
                    >
                      {" "}
                      <QuestionCircleOutlined />{" "}
                ...
                <div className="content">
                  ...
                  <div
                    id="strategiesTreeArea"
                    className="scroll"
                    style={{ height: this.state.contentRightHeight2 }}
                  >
                    <div
                      className="droppable"
                      onDragOver={(e) => this.onDragOver(e)}
