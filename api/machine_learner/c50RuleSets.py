...
def formatTreeRulesToMlResult(x, y):
    ...
    # # Ensure x and y are DataFrame and Series/Numpy array respectively
    # if not (isinstance(x, pd.DataFrame) and (isinstance(y, pd.Series) or isinstance(y, np.ndarray))):
    #     raise ValueError("x must be a pandas DataFrame and y must be a pandas Series or a numpy array")

    # # Convert Python objects to R objects
    # convertedX = pandas2ri.py2rpy(x)
    # convertedY = pandas2ri.py2rpy(y)
